...
export function MCPManagement() {
  ...
  const handleInlineProviderSave = async () => {
    ...
    if (anyActionSelected && providerData.connection_schema && typeof providerData.connection_schema === 'object') {
      const connectionSchema = providerData.connection_schema; // Save to a const
      const shape = connectionSchema?.typeName === 'ZodObject' && connectionSchema.shape ? connectionSchema.shape : null; // Get shape
      const schemaKeys = shape ? Object.keys(shape) : []; // Get keys from shape

      const missingRequiredParam = schemaKeys.find(key => {
        const schemaField = shape![key] as any; // Use shape here
        // Heuristic for required: not Optional and not Default
      ...
      if (missingRequiredParam) {
        const fieldDetails = shape && shape[missingRequiredParam] ? (shape[missingRequiredParam] as any) : null; // Use shape here
    ...
    for (const discoveredAction of providerData.actions) {
      ...
      if (isSelected) {
        const dataToSave = {
          ...
          output_schema: discoveredAction.output_schema || {}, // Include output_schema
  ...
  const handleProviderSelect = async (selectedProviderNameValue: string) => {
    ...
    if (selectedProviderNameValue && selectedProviderNameValue !== 'custom' && user) {
      ...
      try {
        ...
        // Updated to check shape for iteration
    ...
    if (selectedDiscoveredProvider && selectedDiscoveredProvider.actions) {
      selectedDiscoveredProvider.actions.forEach(discoveredAction => {
        ...
        newActionFormsData[discoveredAction.action_name] = {
          ...
          output_schema: discoveredAction.output_schema, // Include output_schema
          args_schema: discoveredAction.args_schema, // Include args_schema
        };
      });
    }
  ...
  const handleSave = async () => {
    ...
    if (anyActionSelected &&
      ...
      const connectionSchema = currentProviderData.connection_schema; // Save to const
      const shape = connectionSchema?.typeName === 'ZodObject' && connectionSchema.shape ? connectionSchema.shape : null; // Get shape
      const schemaKeys = shape ? Object.keys(shape) : []; // Get keys from shape
      ...
      if (missingRequiredParam) {
        const fieldDetails = shape && shape[missingRequiredParam] ? (shape[missingRequiredParam] as any) : null; // Use shape here
        const displayName = fieldDetails?.description || fieldDetails?.display_name || missingRequiredParam; // Prioritize description
        toast({
          title: "Validation Error",
    ...
    for (const actionConfig of Object.values(actionFormsData)) {
      ...
      if (actionConfig.is_selected) {
        ...
        // let parsedSamplePayload = {}; // Not used for saving expected_format anymore
        // try {
        ...
        const dataToSave = {
          ...
          expected_format: actionConfig.args_schema || {}, // Save args_schema to expected_format
          output_schema: actionConfig.output_schema || {}, // Save output_schema
    ...
    if (operations.length === 0 && itemsSaved === 0) { // Check itemsSaved as well
  ...
  const handleEdit = (endpoint: MCPEndpoint) => {
    ...
    setFormData(prev => ({
        ...prev,
        selected_provider_name: endpoint.provider_name || endpoint.category.toLowerCase(), // Use provider_name if available, else derive from category
        mcp_server_base_url: endpoint.mcp_server_base_url || '',
        provider_name: endpoint.provider_name || endpoint.category.toLowerCase(), // Actual provider name
        // auth_token: endpoint.auth_token || '', // This is the default provider key, action specific keys are in actionFormsData - DEPRECATED
    }));
    ...
    const effectiveProviderName = endpoint.provider_name || endpoint.category.toLowerCase();
     if (formData.selected_provider_name === effectiveProviderName) {
        handleProviderSelect(effectiveProviderName);
     } else {
        // Update selected_provider_name, which will trigger handleProviderSelect via the Select component's effect
         setFormData(prev => ({...prev, selected_provider_name: effectiveProviderName}));
     }
            ...
            {formData.selected_provider_name && formData.selected_provider_name !== 'custom' && (
              loadingMainFormConnectionParams ? (
                ...
                (() => {
                  ...
                  } else if (providerData && (!shape || Object.keys(shape).length === 0)) { // Check shape instead of connectionSchema directly for Object.keys
                    return (
                  ...
                  <div className="p-4 border-t border-dashed mt-2 space-y-4">
                    {loadingConnectionParams ? (
                      ...
                      (() => {
                        ...
                        if (shape && Object.keys(shape).length > 0) {
                          return (
                            <div className="space-y-3">
                              ...
                              {Object.entries(shape).map(([key, schemaValue]: [string, any]) => { // Iterate over shape
                                const isRequired = !schemaValue.typeName?.startsWith('ZodOptional') && !schemaValue.typeName?.startsWith('ZodDefault');
                                return (
                                  <div key={key}>
                        ...
                        } else if (providerData && (!shape || Object.keys(shape).length === 0)) { // Check shape instead of connectionSchema
                      ...
                      <TableRow key={endpoint.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex gap-2 justify-end">
                            ...
                            {/* Edit button removed from table row as per new inline provider edit flow */}
                            {/* <Button variant="outline" size="icon" onClick={() => handleEdit(endpoint)}> <Edit className="h-4 w-4" /> </Button> */}
                            <Button
                              variant="destructive"
                              size="icon"
          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                The 'args' will be taken from this payload. The 'auth.token' will use the configured Provider API Key for this provider ({categoryMapUtil[testingEndpoint.provider_name?.toLowerCase() || 'custom'] || testingEndpoint.provider_name}).
              </DialogDescription>
            </DialogHeader>
